...
export default function LayerItem({ component, draggingCmp, dragParent, ...props }: LayerItemProps) {
  ...
  const wrapperCls = cx('layer-item flex flex-col', selected && 'bg-sky-900', (!visible || isDragging) && 'opacity-50');
  ...
  return (
    <div className={wrapperCls}>
      ...
      {!!(open && components.length) && <div className={cx('max-w-full', !open && 'hidden')}>{cmpToRender}</div>}
    </div>
  );
}
